...
// Add a 'bool &branched' parameter
void writeback(mowb &obj5, ipc &obj1, regFile &rf, bool &branched)
{
    if (!(obj5.v))
    {
        branched = false; // No instruction, so no branch
        return;
    }
    ...
    {
        obj1.pc = obj5.jpc;
        branched = true; // A jump happened
    }
    else if (obj5.branch == 1)
    {
        obj1.pc = obj5.bpc;
        branched = true; // A branch was taken
    }
    else
    {
        // This is a sequential instruction.
        // DO NOT update obj1.pc here. Let the main loop do it.
        branched = false;
    }

    // --- The rest of your writeback logic is correct ---
    int wdata;
    if (obj5.jump == 1)
    {
        wdata = obj5.dpc + 1;
    ...
    {
        cout << "!!! WB_STAGE (dpc=" << obj5.dpc << "): Writing " << wdata << " to reg[" << obj5.rdl << "]" << endl;
 ...
 while (obj1.pc < machinecode.size() || obj2.v || obj3.v || obj4.v || obj5.v)
    {
        ...
        int pc_for_fetch = obj1.pc; 
        bool branched = false;      

        writeback(obj5, obj1, RegFile, branched);
        memop(obj4, obj5, MainMem);
        execute(obj3, obj4, alu, ALUControl);
        insdecode(obj2, obj3, ImmGen, ControlPath, RegFile);
        ...
        if (!obj2.s)
        {
            if (pc_for_fetch >= 0 && pc_for_fetch < machinecode.size())
            {
                insfetch(pc_for_fetch, obj2, InsMem);
            }
            else
            {
        ...
        if (!branched && !obj2.s)
        {
            obj1.pc = pc_for_fetch + 1;
        }
    
        cout << "stages :" <<cycle<< endl;
          for (int i = 0; i < 33; i++)
        {
            cout << RegFile.regs[i] << " ";
        }
